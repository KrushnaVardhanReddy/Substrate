		...
		if err := json.Unmarshal(req.DiffReport, &diffReport); err == nil {
			if diffReport.Summary.BreakingCount > 0 && req.Org != "" {
				...
				go func(diffId string, diffReq SaveDiffRequest) {
					...
					var brokenConsumers []webhook.BrokenConsumer
						...
						if err == nil {
							for _, res := range crResp.Results {
								if res.Status == "breaking" {
									brokenConsumers = append(brokenConsumers, webhook.BrokenConsumer{
					...
					event := webhook.BreakingChangeEvent{
						EventID:   eventID,
					...
					webhook.DispatchEvent(bgCtx, store, event)
