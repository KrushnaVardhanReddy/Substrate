...
func SyncHandler(store db.Store, riverClient workers.JobEnqueuer) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		_, err := riverClient.Insert(ctx, workers.SyncWebhookJob{Request: req}, nil)
		if err != nil {
			http.Error(w, `{"error": "internal error enqueuing job"}`, http.StatusInternalServerError)
			return
		}
