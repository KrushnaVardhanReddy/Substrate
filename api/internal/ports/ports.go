...
// Substrate API. Each port represents a distinct domain boundary.
// HTTP handlers (adapters) accept these narrow interfaces, never the concrete
// db.PGStore or the monolithic db.Store, keeping the transport layer
// fully decoupled from the DB implementation.
package ports
...
// DependencyStore is the port for cross-repo dependency tracking.
...
// PoolProvider exposes the raw pgxpool for the rare cases where a specialised
// handler manages its own queries (legacy pattern being phased out).
