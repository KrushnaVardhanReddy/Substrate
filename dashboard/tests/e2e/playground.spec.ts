...
paths:
  ...
  test('should stream AI analysis and display findings', async ({ page }) => {
    ...
    await page.route(ANALYZE_API, async (route) => {
      await route.fulfill({
        ...
        body: 'data: {"finding": "Field userId removed (BREAKING)"}\n\ndata: {"autofix": "openapi: 3.0.0\\ninfo:\\n  title: Fixed API"}\n\ndata: [DONE]\n\n'
      });
    });
    ...
    // c. Assert heading
    await expect(page.getByRole('heading')).toContainText(/Playground|AI|Studio/i);
    ...
    const findings = page.locator('.analysis-result, .findings-panel, [data-testid="findings"]');
  ...
  test('should apply auto-fix to editor content', async ({ page }) => {
    ...
    await page.route(ANALYZE_API, async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'text/event-stream',
        body: 'data: {"finding": "BREAKING CHANGE"}\n\ndata: {"autofix": "openapi: 3.0.0\\ninfo:\\n  title: Fixed Content"}\n\ndata: [DONE]\n\n'
      });
    });
    ...
    // d. Read content
    let content: string;
    if (await page.locator('textarea').first().isVisible()) {
      content = await page.locator('textarea').first().inputValue();
    } else {
      content = await page.locator('[contenteditable="true"]').first().innerText();
    }

    // e. Assert content changed from original
    expect(content).not.toBe(VALID_SCHEMA);
    expect(content).toContain('Fixed Content');
  ...
  test('should handle malformed input gracefully', async ({ page }) => {
    ...
    // e. Assert error visible
    const alert = page.locator('[role="alert"], .error-message, .toast-error');
