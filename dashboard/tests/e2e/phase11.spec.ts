...
test.describe('Phase 11: Graph UI & Visual Studio', () => {
	...
	test('should render graph container and interact with nodes', async ({ page }) => {
		try {
			...
			test.skip(true, 'Graph API not reachable or timed out. Skipping.');
			return;
		}
		...
		try {
			...
			console.log('Cytoscape graph empty or no dependency edges seeded for mcp-org. Skipping node interaction.');
		}
	});
