<script lang="ts">
    ...
    let isGraphEmpty = $derived(
        !debouncedSearch && !showOnlyBreaking && protocolFilter === "All",
    );
    ...
    onMount(() => {
        ...
        const processGraphData = (edgesData: any) => {
            ...
            edgesData.forEach((edge: any) => {
                addNode(
                    ...
                    edge.status,
                    "provider",
                    edge.provider_metadata,
                );
                addNode(
                    edge.consumer,
                    "SAFE",
                    "consumer",
                    edge.consumer_metadata,
                );
            ...
            rawEdges = newEdges;
            console.log(
                "processGraphData finished. rawNodes length:",
                rawNodes.length,
            );
        };

        console.log(
            "Mounting component. data.graphData length:",
        ...
        const fetchGraph = async () => {
            ...
            try {
                ...
                const res = await fetch(`/api/v1/graph/${$page.params.org}`, {
                    headers,
                });
                if (res.ok) {
    ...
    <main class="main-canvas">
        <div class="canvas-header">
            ...
            <p class="page-subtitle">
                Visualizing dependencies for {$page.params.org}
            </p>
        </div>
        ...
        <div
            ...
            style="z-index: 20; display: flex; gap: 8px;"
        >
        ...
        >
            {#if isGraphEmpty}
                <div class="empty-state">
                    ...
                    <p>
                        Enter a service name, or check "Show Only BREAKING
                        Changes" to generate the graph.
                    </p>
                </div>
            {/if}
            <SvelteFlow
                ...
                onnodeclick={(...args: any[]) => {
                    ...
                    const node =
                        args.length > 1
                            ? args[1]
                            : args[0]?.node || args[0]?.detail?.node;
                ...
                <div>
                    ...
                    <div class="detail-meta">
                        <span class="meta-badge"
                            >{selectedNode.version || "v1.0.0"}</span
                        >
...
&#125;</code
                        ...
                        <div class="metadata-grid">
                            {#if selectedNode.data.metadata.team}
                                ...
                                <div class="meta-value">
                                    {selectedNode.data.metadata.team}
                                </div>
                            {/if}
                            {#if selectedNode.data.metadata.type}
                                ...
                                >
                                    {selectedNode.data.metadata.type}
                                </div>
                            {/if}
                            {#if selectedNode.data.metadata.databases && selectedNode.data.metadata.databases.length > 0}
                                ...
                                <div class="meta-value">
                                    {#each selectedNode.data.metadata.databases as db}
                                        <span
                                            style="background: #2D3240; padding: 2px 6px; border-radius: 4px; font-size: 11px; margin-right: 4px;"
                                            >{db}</span
                                        >
                                    {/each}
                                </div>
                ...
                <section class="detail-section">
                    ...
                    <div class="metadata-grid">
                        ...
                        <div
                            class="meta-value {selectedNode.status ===
                            'BREAKING'
                                ? 'error-text'
                                : ''}"
                        >
            ...
            <div class="detail-footer">
                <a
                    href={`https://github.com/${selectedNode.id}/actions`}
                    target="_blank"
                ...
                >
                <a
                    href={`vscode://vscode.git/clone?url=https://github.com/${selectedNode.id}.git`}
                    class="btn-primary"
                    style="display: flex; justify-content: center; align-items: center; text-decoration: none;"
                    >Open in IDE</a
                >
