<script lang="ts">
	...
	let showVolatilityHeatmap = $state(false);

	let tooltip = $state<{ edgeId: null | string; x: number; y: number }>({ edgeId: null, x: 0, y: 0 });
	...
	$effect(() => {
		...
		let blastRadiusIds = new Set<string>();
		if (selectedNode) {
			blastRadiusIds.add(selectedNode.id);
			let added = true;
			while (added) {
				added = false;
				for (const edge of filteredEdges) {
					// Cascading downstream means if source is in blastRadius, target is affected
					if (blastRadiusIds.has(edge.source) && !blastRadiusIds.has(edge.target)) {
						blastRadiusIds.add(edge.target);
						added = true;
					}
				}
			}
		...
		// Apply flags to node data
		filteredNodes = filteredNodes.map(n => ({
			...n,
			data: {
				...n.data,
				blastRadius: blastRadiusIds.has(n.id),
				heatmapActive: showVolatilityHeatmap
			}
		}));
	...
	<main class="main-canvas">
		...
		<div class="graph-controls" style="z-index: 20;">
			<div class="filter-panel">
				...
				<label class="filter-label">
					<input type="checkbox" bind:checked={showVolatilityHeatmap} />
					Volatility Heatmap
				</label>
				<label class="filter-label">
