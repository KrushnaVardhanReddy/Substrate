...
| **P7-T03** | 🟢 P3 | **Hybrid Custom Governance Rules (CEL + WASM)** — Phase 1: Let enterprises define custom schema diffing rules via easy CEL expressions. Phase 2: Support WebAssembly (Wazero) plugins for Turing-complete governance rules written in Rust/Go/TS. | Jules | 💡 Backlog | `docs/specs/phase-7/p7-t03-custom-rules-engine.md` |
...
| **P7-T05** | ⚪ P5 | **Hybrid Runtime Drift Detection (Sidecar + eBPF)** — Phase 1: Deploy a lightweight proxy sidecar to detect un-documented schema payloads. Phase 2: Extend with a zero-latency `cilium/ebpf` kernel probe for high-throughput environments. | Jules | 💡 Backlog | `docs/specs/phase-7/p7-t05-runtime-drift-detection.md` |
...
| **P8-T06** | ⚪ P5 | **Management ROI Dashboard** — A specialized view that calculates the literal engineering hours and monetary value saved by Substrate preventing downstream outages this month. | Jules | 🔄 In Progress (`13233266981728314963`) | `docs/specs/enterprise-vision.md` |
| **P8-T07** | 🔴 P1 | **Billing & Subscription Engine (Paywall Pause)** — Add `trial_ends_at` to the DB, update the GitHub App to enforce Audit Mode during the 90-day trial, and gracefully pause analysis when the trial expires until Stripe checkout. | Jules | 🔄 In Progress (`3385015114758921079`) | `docs/specs/go-to-market-strategy.md` |
...
| **P8-T10** | 🔴 P1 | **Phase 8 E2E Testing** — Test the enterprise readiness components (Job Queue, Authz, Cascading Rollback, Billing) against the real database schema. | Jules | 🔄 In Progress (`4813219911247863374`) | `docs/specs/phase-8/p8-t10-e2e-tests.md` |
...
| **P11-T10** | 🔴 P1 | **Svelte Flow Migration** — Migrate the entire graph visualization from Cytoscape.js to `Svelte Flow` for native reactivity, gorgeous HTML custom nodes, and built-in minimap support. | Stitch | ⏳ Ready to Start | `(Pending)` |
| **P11-T11** | 🔴 P1 | **Global Command Palette (Cmd+K)** — Implement a Raycast-style command palette for instant global search across 100+ microservices, allowing users to jump directly to specific nodes. | Stitch | ⏳ Ready to Start | `(Pending)` |
...
| **P11-T14** | 🔴 P1 | **Premium Aesthetics System** — Overhaul the UI with a strict focus on Enterprise SaaS aesthetics: Deep Dark Mode, Glassmorphism modals, curated HSL color palettes, and edge-flow micro-animations. | Stitch | ⏳ Ready to Start | `(Pending)` |
| **P11-T15** | 🔴 P1 | **Zero-to-One Onboarding Wizard** — Create a frictionless, animated onboarding wizard (Connect GitHub → Scan Repos → Build Graph) to guarantee a flawless 5-minute enterprise onboarding experience. | Stitch | ⏳ Ready to Start | `(Pending)` |
