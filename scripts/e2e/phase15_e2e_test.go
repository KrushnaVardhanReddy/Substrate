...
func waitForP15Services(t *testing.T) {
	...
	t.Fatalf("API server not reachable at %s. Please ensure 'make api' and 'make postgres' are running.", p15ApiURL)
}
...
func setupP15Database(t *testing.T) (*pgxpool.Pool, string, string) {
	...
	require.NoError(t, err)
	_, err = pool.Exec(ctx, "DELETE FROM marketplace_plugins")
	require.NoError(t, err)
	...
	var orgID string
	err = pool.QueryRow(ctx, "INSERT INTO organizations (github_installation_id, github_org_name, is_oss) VALUES (1515, 'acme', true) RETURNING id").Scan(&orgID)
	require.NoError(t, err)
	...
	_, err = pool.Exec(ctx, "INSERT INTO insurance_claims (id, org_id, policy_id, github_pr_url, incident_date, status, amount_cents) VALUES ($1, $2, $3, $4, NOW(), $5, $6)", uuid.New().String(), orgID, policyID, "https://github.com/acme/repo/pull/10", "APPROVED", 15000)
	require.NoError(t, err)

	// Create marketplace plugin
	pluginID := uuid.New().String()
	pluginSchema := `{"rules": [{"id": "hipaa-rule", "description": "Ensure HIPAA compliance"}]}`
	_, err = pool.Exec(ctx, "INSERT INTO marketplace_plugins (id, name, description, schema_content) VALUES ($1, $2, $3, $4)",
		pluginID, "substrate-plugin-hipaa", "HIPAA Plugin", pluginSchema)
	require.NoError(t, err)

	return pool, orgID, policyID
}

func TestPhase15SystemE2E(t *testing.T) {
	if testing.Short() {
	...
	t.Run("Scenario 2: Marketplace Plugin Install", func(t *testing.T) {
		...
		var count int
		err = pool.QueryRow(context.Background(), "SELECT COUNT(*) FROM marketplace_plugins WHERE name = $1", "substrate-plugin-hipaa").Scan(&count)
		require.NoError(t, err)
		assert.Greater(t, count, 0, "Plugin should exist in DB")
		// Query DB to assert plugin exists
	})

	t.Run("Scenario 3: Public Profile API", func(t *testing.T) {
		req, err := http.NewRequest("GET", p15ApiURL+"/api/v1/public/profile/acme", nil)
		require.NoError(t, err)
		...
		require.Contains(t, respBody, "metrics")
		metrics := respBody["metrics"].(map[string]interface{})
		assert.Contains(t, metrics, "breaking_changes")
	...
	t.Run("Scenario 4: Schema Insurance Claims", func(t *testing.T) {
		...
		assert.Equal(t, "https://github.com/acme/repo/pull/42", claim["github_pr_url"])
