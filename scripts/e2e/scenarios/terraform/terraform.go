...
func RunAll(ctx context.Context, client *github.Client, owner string) {
	log.Println("Starting Terraform Automated Test Suite...")
...
func RunSafe(ctx context.Context, client *github.Client, owner string) {
	executeScenario(ctx, client, owner, "Terraform Safe Extension",
...
func RunBreaking(ctx context.Context, client *github.Client, owner string) {
	executeScenario(ctx, client, owner, "Terraform Breaking Change",
		defaultSubstrateYaml(), defaultSchema(),
		defaultSubstrateYaml(), breakingSchema(),
		"BREAKING", "TF_OUTPUT_REMOVED")
}

func RunOverride(ctx context.Context, client *github.Client, owner string) {
	executeScenario(ctx, client, owner, "Terraform Config Override",
		defaultSubstrateYaml(), defaultSchema(),
...
func executeScenario(ctx context.Context, client *github.Client, owner, scenarioName, baseConfig, baseSchema, prConfig, prSchema, expectedStatus, expectedRule string) {
	...
	helpers.SeedFile(ctx, client, owner, providerRepoName, "plan.json", "main", baseSchema)
...
consumers:
  - name: "test-provider"
	...
	helpers.SeedFile(ctx, client, owner, consumerRepoName, "plan.json", "main", baseSchema)

	log.Println("Waiting 15s for the Registry API to sync baselines...")
	time.Sleep(15 * time.Second)

	prBranch := fmt.Sprintf("e2e-terraform-%d", time.Now().Unix())
	...
	helpers.SeedFile(ctx, client, owner, providerRepoName, "plan.json", prBranch, prSchema)
...
func defaultSubstrateYaml() string {
	return `service: test-provider-tf
schema_type: terraform-plan
spec_path: plan.json
`
}
...
overrides:
  - rule_id: TF_OUTPUT_REMOVED
    path: my_bucket_arn
    reason: "E2E testing TF override functionality"
...
func defaultSchema() string {
	// Base is a plan that creates an output and doesn't delete anything
	return `{
  "resource_changes": [],
  "output_changes": {}
}`
...
func safeSchema() string {
	// Plan that creates a new resource (safe)
	return `{
  "resource_changes": [
    {
