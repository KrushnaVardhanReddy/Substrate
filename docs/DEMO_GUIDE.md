...
*“Eliminate brittle test flakiness and phantom failures caused by unannounced contract drift.”*

- **Zero 'Phantom' Failures in Staging:** QA engineers spend hours investigating red test runs only to find an upstream team quietly renamed a field or removed an enum. Substrate blocks breaking changes at the PR gate before bad code ever lands in QA/staging environments.
- **Service-Level Blast Radius for Test Scoping:** When a backend service changes, QA doesn't have to guess or run the entire company's regression suite. The Substrate dependency graph shows exactly which downstream consumer services depend on that schema, allowing QA to pinpoint which integration suites to trigger.
- **Contract & Mock Synchronization:** Validates that client-side mocks, Postman collections, and Cypress/Playwright stubs align with the verified upstream schema specification rather than stale assumptions.
- **Instant Release Changelogs:** Generates clean markdown changelogs directly from schema diffs (`substrate diff --format=changelog`), giving QA an instant, accurate inventory of all added, modified, or deprecated fields for test planning.
...
**Show the generated `substrate.yaml` configs:**

It's important to explain that `substrate.yaml` serves two distinct purposes depending on whether the team is *providing* an API or *consuming* one (a single repo can do both!):

**1. The Provider Configuration (e.g. backend API)**
```yaml
# substrate.yaml
...
# Taxonomy metadata (Powers the Graph UI & MCP Context)
metadata:
  type: backend
  team: payments-team
  slack_channel: "#alerts-payments-l2"
  pagerduty: "PD-PAYMENTS-API"
  sla_tier: "Tier-1-Critical"

overrides: []
```

**2. The Consumer Configuration (e.g. frontend app)**
```yaml
# substrate.yaml
service: checkout-ui

# This repo doesn't own an API, but it depends on the payments API
consumers:
  - name: "payments-api"
    provider_repo: company/payments-api
    schema_type: openapi
