...
- **Database Collision Fix**: When `handler.go` discovers a new provider repository, it MUST generate a negative pseudo-ID (e.g., using `crc32` of the `fullName`) instead of passing `0` for the `github_repo_id` to `store.UpsertRepo`. This prevents PostgreSQL from colliding all newly discovered providers into a single row due to the `github_repo_id` unique constraint.
