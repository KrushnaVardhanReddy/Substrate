...
## Requirements
1. **Test Environments**: Use Testcontainers for Postgres and a local Forgejo/Gitea instance for webhook integration tests.
2. **Coverage**: Tests must cover the AI Mock Data Generator, Spectral Linter, Auto-SDK Generator, and Zombie API detection.
3. **No Mocks**: All external service calls (Forgejo, Stripe) must use sandboxed/test-mode credentials, never mock interceptors.
