...
## 1. Overview
Migrate the primary dependency graph visualization from SvelteFlow back to `cytoscape` (Cytoscape) for better reactivity, custom HTML nodes, and stable performance with massive enterprise graphs.

## 2. Requirements
- Replace SvelteFlow with modern Cytoscape integrations using Svelte 5 `$effect` for reactivity.
- Implement custom styling for nodes that displays the service name below the node (`text-valign: bottom`, `text-margin-y: 8`). Nodes MUST utilize dynamic Lucide icons (e.g., AppWindow, Database, Smartphone, Server) embedded as SVG data URIs via `background-image` in Cytoscape styles. Use a tinted background (`background-color`) matching the SVG stroke color (Cyan for Database, Purple for Frontend, Rose for Mobile, Emerald for Backend). Nodes should be a fixed `48x48` size with the SVG centered.
...
- Implement custom Zoom Controls (`+`, `-`, `Fit`) as overlay buttons over the graph canvas.
- **Default Exploration Model:** To allow full visibility, the initial graph state MUST render all nodes and edges by default.
- **Symmetrical Recursive Filtering:** When generating the sub-graph based on a search or filter, the logic MUST recursively pull in ALL layers of upstream providers (dependencies) and ALL layers of downstream consumers (blast radius). This guarantees that the entire cascading flow of a breaking change is available on the canvas for highlighting, enabling full end-to-end tracing (e.g., from frontend all the way down to database).
...
- **Reactivity Considerations (Svelte 5):** When implementing debounce functionality for the search input using `$effect`, reactive dependencies (e.g., `searchQuery`) MUST be read synchronously before any async callback (like `setTimeout`). Failure to do so prevents Svelte 5 from tracking the dependency.
