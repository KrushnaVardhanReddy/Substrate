...
## 2. Requirements
- Modify the existing Svelte Flow Custom Node to accept an `isActive` and `isFaded` state.
- When a node is clicked, run a DFS/BFS traversal on the Svelte Flow edges to find all downstream dependents (1st, 2nd, Nth degree).
...
- **Jules:** Implement the traversal logic in Svelte 5 and update the Svelte Flow store. No Stitch mockup needed since this is purely a visual state change of existing nodes.
