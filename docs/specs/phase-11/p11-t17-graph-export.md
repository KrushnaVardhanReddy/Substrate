...
### 2.1 Dependency
- The project MUST use the `html-to-image` package to convert the Cytoscape DOM viewport into a data URL.
...
### 2.3 Export Logic
- When clicked, the function MUST target the `.cytoscape__viewport` node.
- It MUST optionally trigger Cytoscape's `fitView` before capture to ensure all nodes are cleanly in the frame.
